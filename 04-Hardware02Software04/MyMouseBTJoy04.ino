...
void loop()
  ...
  mouseRefresh=false;
  x2=joystickSteperMouse(horizontal2);
  y2=joystickSteperMouse(vertical2);
     
 
  if((x2!=0)||(y2!=0)) mouseRefresh=true;
  //botão1 acionado
  if((digitalRead(SEL2)==0)&&(button1==false)) {button1=true;mouseRefresh=true;}//manda a informação de botão acionado uma vez
  //botão1 aberto
  if((digitalRead(SEL2)==1)&&(button1==true)) {button1=false;mouseRefresh=true;}//manda a informação de botão levantado uma vez
  
  if(mouseRefresh) {mouseStep2();digitalWrite(redLed, HIGH );digitalWrite(greenLed, HIGH);digitalWrite(blueLed, HIGH);}
